...
class GKSexViewController: BaseConnectionController {
    ...
    private lazy var listData: [RoubSiteNovelBlockInfo] = {
        return []
    }()
    private lazy var selectData: [RoubSiteNovelBlockInfo] = {
        return []
    }()
    ...
    override func refreshData(page: Int) {
        RoubSiteBlockNet.getAllBlocks(sucesss: { (object) in
            if ("1" == object["status"].stringValue) {
                var blockList: [RoubSiteNovelBlockInfo] = [];
                let data: [JSON] = object["data"].arrayValue
                for item in data {
                    let block: RoubSiteNovelBlockInfo = RoubSiteNovelBlockInfo.init();
                    block.title = item["BLOCK_NAME"].stringValue;
                    block.blockId = item["BLOCK_ID"].stringValue;
                    block.shortTitle = item["BLOCK_NAME"].stringValue;
                    blockList.append(block);
                }
                var info: GKRankInfo = GKRankInfo.init();
                info.female = blockList;
                info.male = blockList;
                self.info = info;
    ...
    @objc private func sureAction() {
        ...
        if completion != nil {
            ...
            RoubSiteBlockNet.reloadHomeData(options: .DataNet);
            self.goBack();
        }
    }
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let info: RoubSiteNovelBlockInfo = self.listData[indexPath.row];
        cell.model = info;
        return cell;
    }
    ...
    override func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let content = self.selectData.contains { (model) -> Bool in
            return model.blockId == info.blockId;
        }
        if content {
